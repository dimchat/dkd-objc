...
@implementation DKDKeyStore (CacheFile)
...
- (BOOL)flush {
    ...
    // keys from contacts
    path = [self _pathWithID:ID filename:DKD_KEYSTORE_ACCOUNTS_FILENAME];
    BOOL OK1 = [self.keysFromAccounts writeToBinaryFile:path];
    
    // keys from group.members
    path = [self _pathWithID:ID filename:DKD_KEYSTORE_GROUPS_FILENAME];
